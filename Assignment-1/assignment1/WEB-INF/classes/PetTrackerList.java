...
public class PetTrackerList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
		   if(CategoryName.equals("fitbark"))
		   {
		   ...
		   else if(CategoryName.equals("cattailer"))
		    {
			for(Map.Entry<String,PetTracker> entry : SaxParserDataStore.petTrackers.entrySet())
				{
				 if(entry.getValue().getRetailer().equals("Cat Tailer"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("petbiz"))
			{
				for(Map.Entry<String,PetTracker> entry : SaxParserDataStore.petTrackers.entrySet())
				{
				 if(entry.getValue().getRetailer().equals("Petbiz"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("petfon"))
			{
				for(Map.Entry<String,PetTracker> entry : SaxParserDataStore.petTrackers.entrySet())
				{
		...
		for(Map.Entry<String, PetTracker> entry : hm.entrySet())
		{
			...
			System.out.println("+++++++++++++++++++++++++++++++++++"+petTracker.getName());
			pw.print("<strong>$"+petTracker.getPrice()+"</strong><ul>");
			pw.print("<li id='item'><img src='images/petTrackers/"+petTracker.getImage()+"' alt='' /></li>");

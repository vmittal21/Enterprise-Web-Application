...
public class Carousel{
	...
	public String  carouselfeature(Utilities utility){
		...
		// HashMap<String, Console> con = new HashMap<String, Console>();
		// HashMap<String, Laptop> lp = new HashMap<String, Laptop>();
		
		// HashMap<String, HeadPhone> hph = new HashMap<String, HeadPhone>();
		// HashMap<String, SmartWatch> sw = new HashMap<String, SmartWatch>();
		
		// HashMap<String, VoiceAssistant> va = new HashMap<String, VoiceAssistant>();
		HashMap<String, Tablet> tt = new HashMap<String, Tablet>();
		// HashMap<String, FitnessWatch> fw = new HashMap<String, FitnessWatch>();
		...
		if(CategoryName==null){
			try{
			// con=MySqlDataStoreUtilities.getConsoles();
			// lp=MySqlDataStoreUtilities.getLaptops();
			// hph=MySqlDataStoreUtilities.getHeadPhones();
			// sw=MySqlDataStoreUtilities.getSmartWatches();
			// va=MySqlDataStoreUtilities.getVoiceAssistants();
			tt=MySqlDataStoreUtilities.getTablets();
			// fw=MySqlDataStoreUtilities.getFitnessWatches();
			name = "";
			}catch(Exception e)
			{
		...
		for(String user: prodRecmMap.keySet())
		{
			...
			if(user.equals(utility.username()))
			{
				...
				sb.append("<link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css' integrity='sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm' crossorigin='anonymous'>");
...
sb.append("<script src='https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js' integrity='sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl' crossorigin='anonymous'></script>");
				...
				*/
				sb.append("<div class='carousel-inner' role='listbox' style=' width:100%; height: 360px !important;'>");
				...
				for(String prod : productsList){
					...
					if (k==0 )
					{
						sb.append("<div class='item active'><div class='col-md-6' style = 'background-color: #58acfa;display: inline-block; border :1px solid #cfd1d3'>");
					}
					else
					{
						sb.append("<div class='item'><div class='col-md-6' style = 'background-color: #58acfa ;display: inline-block; border :1px solid #cfd1d3'>");
					}
				...
				*/
				sb.append("<a class='left carousel-control' href='#"+myCarousel+"' data-slide='prev' style = 'width : 10% ;background-color:#611616; opacity :1'>"+
						"<span class='glyphicon glyphicon-chevron-left' style = 'color: red'></span>"+
						"<span class='sr-only'>Previous</span>"+
						"</a>");
				sb.append("<a class='right carousel-control' href='#"+myCarousel+"' data-slide='next' style = 'width : 10% ;background-color:#611616; opacity :1'>"+
